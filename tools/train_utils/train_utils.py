...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    # --- Epoch 누적용 변수 선언 ---
    sr_loss_total = 0.0
    sr_pixel_loss_total = 0.0
    det_loss_total = 0.0
    total_loss_total = 0.0
    sr_count = 0

    # 🔍 Object-aware 디버깅 변수 초기화
    object_region_loss_total = 0.0
    background_region_loss_total = 0.0
    object_mask_coverage_total = 0.0
    object_mask_pixel_count = 0
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if 'range_in' in batch:
            with torch.cuda.amp.autocast(enabled=use_amp):
                batch['range_res'] = model.tulip_backbone(batch['range_in'])
                print(f"🟢 TULIP Output Shape in `train_one_epoch`: {batch['range_res'].shape}")
        ...
        if 'loss_sr' in tb_dict:
            sr_loss_total += tb_dict['loss_sr']
            sr_pixel_loss_total += tb_dict.get('pixel_loss', tb_dict['loss_sr'])
            sr_count += 1

        # 🔍 Object-aware 디버깅 수집
        if 'object_region_loss' in tb_dict and 'background_region_loss' in tb_dict:
            object_region_loss_total += tb_dict['object_region_loss']
            background_region_loss_total += tb_dict['background_region_loss']

        if 'object_mask_ratio' in tb_dict and 'object_mask_pixels' in tb_dict:
            object_mask_coverage_total += tb_dict['object_mask_ratio']
            object_mask_pixel_count += tb_dict['object_mask_pixels']

        if 'loss_detection' in disp_dict:
            det_loss_total += disp_dict['loss_detection']
    ...
    if rank == 0:
        if sr_count > 0:
            sr_loss_avg = sr_loss_total / sr_count
            sr_pixel_avg = sr_pixel_loss_total / sr_count
        else:
            sr_loss_avg = sr_pixel_avg = 0.0

        det_loss_avg = det_loss_total / total_it_each_epoch
        total_loss_avg = total_loss_total / total_it_each_epoch

        print("=== [TULIP SR Debug Info] ===")
        print(f"[SR LOSS]   L1 loss = {sr_loss_avg:.6f}, pixel loss = {sr_pixel_avg:.6f}")
        print("=================================")
        print("=== [Joint Training Debug Info] ===")
        print(f"Loss (Detection): {det_loss_avg:.6f}")
        print(f"Loss (Super-Resolution): {sr_loss_avg:.6f} (λ=0.1)")
        print(f"Total Loss: {total_loss_avg:.6f}")
        print("===================================")

        if object_mask_pixel_count > 0:
            avg_obj_loss = object_region_loss_total / sr_count
            avg_bg_loss = background_region_loss_total / sr_count
            avg_obj_mask_ratio = object_mask_coverage_total / sr_count

            print("=== [Object-aware SR Debug - Epoch Avg] ===")
            print(f"Object Region Loss    : {avg_obj_loss:.4f}")
            print(f"Background Region Loss: {avg_bg_loss:.4f}")
            print("============================================")

            print("=== [Object Mask Debug Info - Epoch Avg] ===")
            print(f"Avg Object Mask Coverage: {avg_obj_mask_ratio * 100:.2f}%")
            print("============================================")
